...
class CRM_application:
    def __init__(self, visualize_graph: bool = True, outgoing_mailbox: str = "outgoing_mailbox"):
        ...
        self.graph = Graph(self.llm, self.outgoing_mailbox).build()
        if visualize_graph:
            try:
    ...
    def run(self):
        if not os.path.exists(f"{self.local_dir}/incoming_messages"):
            ...
            incoming_mailbox = f"{self.local_dir}/incoming_mailbox"
